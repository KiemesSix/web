...
function DefaultCaptionButton({ tooltip }) {
  const { translations } = useDefaultLayoutContext(), $ccOnText = $i18n$1(translations, "Phụ đề (c) Bật"), $ccOffText = $i18n$1(translations, "Phụ đề (c) Tắt");
  return html`
    <media-tooltip class="vds-caption-tooltip vds-tooltip">
      <media-tooltip-trigger>
        <media-caption-button
          class="vds-caption-button vds-button"
          aria-label=${$i18n$1(translations, "Phụ đề")}
        >
...
function DefaultPIPButton() {
  const { translations } = useDefaultLayoutContext(), $enterText = $i18n$1(translations, "Trình phát thu nhỏ (i)"), $exitText = $i18n$1(translations, "Mở rộng (i)");
  return html`
...
function DefaultCaptions() {
  ...
  return html`
    <media-captions
      class="vds-captions"
      .exampleText=${$i18n$1(translations, "Chú thích trông như thế này")}
    ></media-captions>
  `;
}
...
const FONT_TEXT_SHADOW_OPTION = {
  type: "radio",
  values: ["Không có", "Bóng đổ rơi", "Được nâng lên", "Chìm", "Đồng nhất"]
};
const FONT_DEFAULTS = {
  fontFamily: "pro-sans",
  fontSize: "100%",
...
function getFontFamilyCSSVarValue(value) {
  switch (value) {
    ...
    case "pro-sans":
      return 'Inter, Roboto, "Arial Unicode Ms", Arial, Helvetica, Verdana, "PT Sans Caption", sans-serif';
    case "casual":
  ...
  return $signal(() => {
    ...
    return html`
      <media-menu class="vds-font-menu vds-menu">
        ${DefaultMenuButton({
      label: () => i18n$1(translations, "Kiểu phụ đề")
    })}
        <media-menu-items class="vds-menu-items">
          ${[
      DefaultMenuSection({
        label: $i18n$1(translations, "Phông chữ"),
        children: [DefaultFontFamilyMenu(), DefaultFontSizeSlider()]
      }),
      DefaultMenuSection({
        label: $i18n$1(translations, "Văn bản"),
        children: [
      ...
      DefaultMenuSection({
        label: $i18n$1(translations, "Nền văn bản"),
        children: [DefaultTextBgInput(), DefaultTextBgOpacitySlider()]
      }),
      DefaultMenuSection({
        label: $i18n$1(translations, "Tô sáng văn bản"),
...
function DefaultResetMenuItem() {
  const { translations } = useDefaultLayoutContext(), $label = () => i18n$1(translations, "Khôi phục");
  return html`
  ...
  return $signal(() => {
    ...
    return html`
      <media-menu class="vds-accessibility-menu vds-menu">
        ${DefaultMenuButton({
      label: () => i18n$1(translations, "Khả năng truy cập"),
...
function DefaultAnnouncementsMenuCheckbox() {
  const { userPrefersAnnouncements, translations } = useDefaultLayoutContext(), label = "Thông cáo";
  return DefaultMenuItem({
  ...
  return $signal(() => {
    ...
    return html`
      <media-menu class="vds-audio-menu vds-menu">
        ${DefaultMenuButton({
      label: () => i18n$1(translations, "Âm thanh"),
      icon: "menu-audio"
    })}
        <media-menu-items class="vds-menu-items">
...
function DefaultAudioBoostSection() {
  return $signal(() => {
    ...
    return DefaultMenuSection({
      label: $i18n$1(translations, "Khếch đại"),
...
function DefaultAudioGainSlider() {
  const { translations } = useDefaultLayoutContext(), $label = $i18n$1(translations, "Khếch đại"), $min = getGainMin, $max = getGainMax, $step = getGainStep;
  return html`
...
function DefaultCaptionsMenu() {
  return $signal(() => {
    const { translations } = useDefaultLayoutContext(), { hasCaptions } = useMediaState(), $offText = $i18n$1(translations, "Tắt");
    if (!hasCaptions()) return null;
    return html`
...
function DefaultSpeedMenuSection() {
  return $signal(() => {
    ...
    return DefaultMenuSection({
      ...
      value: $signal(
        () => playbackRate() === 1 ? i18n$1(translations, "Chuẩn") : playbackRate() + "x"
      ),
      children: [
        DefaultMenuSliderItem({
...
function formatSpokenTime(duration) {
  ...
  if (hours > 0) {
    spokenParts.push(`${hours} giờ`);
  }
  if (minutes > 0) {
    spokenParts.push(`${minutes} phút`);
  }
  if (seconds > 0 || spokenParts.length === 0) {
    spokenParts.push(`${seconds} giây`);
  }
...
class MediaAnnouncer extends Component {
  ...
  #watchCaptions() {
    ...
    this.#setLabel(textTrack() ? "Phụ đề (c) Bật" : "Phụ đề (c) Tắt");
  }
  #watchVolume() {
...
function SpeedMenu() {
  return Menu({ label: "Tốc độ phát", children: SpeedRadioGroup() });
}
...
function CaptionsMenu() {
  return Menu({ label: "Phụ đề", children: CaptionsRadioGroup() });
}
